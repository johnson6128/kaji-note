...
AI 関数（`ai-generate-steps` / `ai-suggest-next-step` / `ai-describe-photo` / `ai-format-voice-text`）は Supabase Auth の JWT が必要。  
`supabase.functions.invoke` は `Authorization: Bearer <jwt>` を自動付与する。

`get-note-by-share-token` は **JWT 不要**（`anon` キーのみで呼び出す）。詳細は当該セクションを参照。

### リクエスト / レスポンス形式

- Content-Type: `application/json`（UC-07-3 も画像は `image_base64` フィールドで JSON 送信）
...
```json
{
  "suggestion": "スポンジでバスタブの側面を円を描くようにこする"
}
```

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `suggestion` | string | 次ステップ候補（1件）。ユーザーがそのまま編集・確定する |
...
## UC-05-2 閲覧専用リンクによるノート取得

**Function name**: `get-note-by-share-token`

認証不要の特殊エンドポイント。`anon` キーで呼び出し可能（JWT 不要）。  
Edge Function が `service_role` でトークン検証・データ取得を行い、未ログインのブラウザからでも閲覧できる。

### リクエスト

```json
{
  "token": "a1b2c3d4e5f67890abcdef1234567890abcd"
}
```

| フィールド | 型 | 必須 | 制約 |
|-----------|-----|------|------|
| `token` | string | ✓ | 36文字の hex 文字列（`gen_random_bytes(18)` の hex エンコード。ダッシュなし） |
    ...
    "steps": [
      {
        ...
        "body": "換気扇を回す",
        "photos": [
          { "id": "uuid", "storage_path": "step-photos/…/….jpg", "position": 1 }
        ]
      }
    ]
  }
...
| code | HTTP | 説明 |
|------|------|------|
| `NOT_FOUND` | 404 | トークンが存在しない・無効化済み・期限切れ、または手順書が `draft` 状態 |
| `INTERNAL_ERROR` | 500 | その他 |

- **認証**: `Authorization` ヘッダー不要。`anon` キーのみで呼び出す
- **下書き**: `note.status = 'draft'` の場合はトークンが有効でも `NOT_FOUND` を返す
- **写真 URL**: レスポンスの `storage_path` はパスのみ。クライアント側で署名付き URL に変換して表示する
