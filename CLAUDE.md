...
| ネットワーク状態監視 | @react-native-community/netinfo |
...
| 関数名 | 用途 |
|--------|------|
| `ai-generate-steps` | UC-07-1: タイトル → ステップ自動生成 |
| `ai-suggest-next-step` | UC-07-2: 次ステップ提案 |
| `ai-describe-photo` | UC-07-3: 写真 → ステップ説明文生成 |
| `ai-format-voice-text` | UC-07-4: 音声テキスト整形 |
| `get-note-by-share-token` | UC-05-2: 未認証ユーザーへの閲覧専用リンクアクセス（service_role でトークン検証） |
...
詳細なセットアップ手順・EAS Build・CI/CD フローは `docs/release-workflow.md` を参照。
...
| `docs/storage-design.md` | Storage バケット設計・パス命名規則・RLS ポリシー・クライアント実装ガイド |
| `docs/edge-functions-api.md` | Edge Functions の入出力インターフェース定義 |
| `docs/error-handling.md` | エラー分類・UX パターン・リトライ戦略・エラーメッセージ一覧 |
| `docs/release-workflow.md` | ブランチ戦略・EAS Build・CI/CD・リリースチェックリスト |
| `supabase/migrations/` | 実際の SQL マイグレーション |

実装開始後に追記してください:
- ディレクトリ構成と各層の責務
- 写真アップロードのフロー（AI用圧縮 → Edge Function → Storage用圧縮 → アップロード → DB）
