...
export default function EditNoteScreen() {
  ...
  const updateNote = useUpdateNote(id ?? null);
  ...
  useEffect(() => {
    ...
    setFrequencyType(note.frequency_type);
    setSteps(
      note.steps.map((s) => ({
        id: s.id,
        position: s.position,
  ...
  function addStep() {
    ...
    setSteps([...steps, { position: steps.length + 1, body: '', newPhotoUris: [] }]);
  }
  ...
  async function pickPhoto(stepIndex: number) {
    if ((steps[stepIndex].newPhotoUris.length ?? 0) >= 3) {
    ...
    next[stepIndex] = {
      ...next[stepIndex],
      newPhotoUris: [...next[stepIndex].newPhotoUris, result.assets[0].uri],
    };
    setSteps(next);
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      const existingIds = new Set(note.steps.map((s) => s.id));
      const keptIds = new Set(steps.filter((s) => s.id).map((s) => s.id!));

      for (const existing of note.steps) {
        if (!keptIds.has(existing.id)) {
          await deleteStep.mutateAsync({ stepId: existing.id, noteId: note.id });
        }
      }
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        if (s.id && existingIds.has(s.id)) {
          await updateStep.mutateAsync({
            stepId: s.id,
            noteId: note.id,
            input: { body: s.body.trim(), position: newPosition },
          });
          const stepId = s.id;
          for (let j = 0; j < s.newPhotoUris.length; j++) {
            const path = await uploadPhoto(s.newPhotoUris[j], note.id, i, j);
  ...
  return (
    <SafeAreaView className="flex-1 bg-cream">
      <View className="flex-row items-center justify-between px-4 py-3 border-b border-ruled-line">
        ...
        <Text className="text-ink font-bold text-base">手順書を編集</Text>
        <TouchableOpacity
          onPress={handleSave}
          disabled={saving}
      ...
      >
          ...
          <View className="flex-row flex-wrap gap-2">
            {CATEGORIES.map((cat) => (
              <TouchableOpacity
                ...
                className={`px-3 py-2 rounded-lg border ${category === cat.value ? 'bg-accent border-accent' : 'bg-white border-ruled-line'}`}
              >
          ...
          <View className="flex-row gap-2">
            {(['draft', 'published'] as NoteStatus[]).map((s) => (
              <TouchableOpacity
                key={s}
                onPress={() => setStatus(s)}
                className={`flex-1 py-2 rounded-lg border items-center ${status === s ? 'bg-accent border-accent' : 'bg-white border-ruled-line'}`}
              >
          ...
          <View className="flex-row flex-wrap gap-2">
            {FREQUENCIES.map((f) => (
              <TouchableOpacity
                ...
                className={`px-3 py-2 rounded-lg border ${frequencyType === f.value ? 'bg-accent border-accent' : 'bg-white border-ruled-line'}`}
              >
        ...
        <View>
          <Text className="text-ink font-semibold text-base mb-3">ステップ ({steps.length}/30)</Text>
          ...
          {steps.map((step, index) => (
            <View key={index} className="mb-3 bg-white border border-ruled-line rounded-xl p-3">
              ...
              <TextInput
                value={step.body}
                onChangeText={(t) => updateStepBody(index, t)}
                multiline
                maxLength={500}
              ...
              >
                <Text className="text-ink-light text-xs">
                  📷 写真を追加 ({step.newPhotoUris.length}/3)
                </Text>
              </TouchableOpacity>
            </View>
          ))}
